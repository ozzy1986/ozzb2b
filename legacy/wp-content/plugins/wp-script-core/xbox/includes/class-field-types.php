	...
	*/
	private function build_file_item( $preview_size, $value, $multiple, $attachment_field, $index = null ){
		...
		if( $this->is_image_file( $value ) ){
			...
			$format = end($format);
			$source_website = '';
			if( strpos($value, 'pornhub.com') > 0 )         $source_website = 'pornhub';
			if( strpos($value, 'redtube.com') > 0 )         $source_website = 'redtube';
			if( strpos($value, 'spankwire.com') > 0 )       $source_website = 'spankwire';
			if( strpos($value, 'tube8.com') > 0 )           $source_website = 'tube8';
			if( strpos($value, 'xhamster.com') > 0 )        $source_website = 'xhamster';
			if( strpos($value, 'xvideos.com') > 0 )         $source_website = 'xvideos';
			if( strpos($value, 'youporn.com') > 0 )         $source_website = 'youporn';
			if( strpos($value, 'drive.google.com') > 0 )    $source_website = 'google_drive';
			if( strpos($value, 'youtube.com') > 0 )         $source_website = 'youtube';
			if( ! empty( $attachment_id ) ){				
				$width = (int) $preview_size['width'];
				$height = ( $preview_size['height'] == 'auto' ) ? $width : (int) $preview_size['height'];
				$item_body = wp_get_attachment_image( $attachment_id, array( $width, $height ), false, array( 'class' => 'xbox-image xbox-preview-handler' ) );
	...
	*/
	...
	public function is_video_file( $file_path = '' ){
		...
		if( $extension && in_array( $extension, array( 'flv', 'mp4', 'webm' ) ) ){
			return true;
		}else if( strpos($video_url, 'pornhub.com') > 0 || strpos($video_url, 'redtube.com') > 0 || strpos($video_url, 'spankwire.com') > 0 || strpos($video_url, 'tube8.com') > 0 || strpos($video_url, 'xhamster.com') > 0 || strpos($video_url, 'xvideos.com') > 0 || strpos($video_url, 'youporn.com') > 0 || strpos($video_url, 'drive.google.com') > 0 || strpos($video_url, 'youtube.com') > 0 ){
			return true;
		}
		return false;
	}
