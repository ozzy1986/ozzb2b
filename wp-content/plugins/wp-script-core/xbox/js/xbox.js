window.XBOX = (function(window, document, $){
  ...
  xbox.init = function() {
    ...
    //$xbox.on('click', '.wpst_import_dummy_content', xbox.on_click_import_dummy_content_values );
  ...
  /*xbox.on_click_import_dummy_content_values = function( event ){
    var $btn = $(this);
    var $xbox_form = $btn.closest('.xbox-form');
    if( ! $xbox_form.length ){
      $xbox_form = $btn.closest('form#post');
    ...
    $.xboxConfirm({
      title: XBOX_JS.text.import_dummy_content_popup.title,
      content: XBOX_JS.text.import_dummy_content_popup.content,
      confirm_class: 'xbox-btn-pink wpst_import_dummy_content_accept',
      confirm_text: XBOX_JS.text.popup.accept_button,
      cancel_text: XBOX_JS.text.popup.cancel_button,
      onConfirm: function(){
        $xbox_form.prepend('<input type="hidden" name="' + $btn.attr('name') + '" value="true">');
  ...
  xbox.add_group_item = function( $btn ){
    ...
    //Active new item
    var $group_control = $btn.closest('.xbox-type-group').find('.xbox-group-control').first();
    $group_control.children('.xbox-group-control-item').eq( index + 1 ).trigger('click');
